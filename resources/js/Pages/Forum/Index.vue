        ...
        <div class="mobile:mr-4 mr-6 md:hidden lg:block">
          <div class="select-wrap">
            <select v-model="filter" @change="Filter"
              ...
              <option v-for="(chanel, index) in chanels" :key="index" :value="chanel.title">{{ chanel.title }}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        <div class="w-full md:w-auto md:mr-6 flex items-center md:block mb-4 md:mb-0">
          ...
          <div class="flex md:hidden items-center justify-center ml-auto">
            <span class="mr-1 btn btn-channel rounded-md ml-5 block w-24 py-1 px-2 text-center text-xs is-eloquent"
              :style="`color: ${post.chanel.color}; border-color: ${post.chanel.color} `">{{ post.chanel.title
              }}s</span>
        ...
        <div class="w-full md:pr-10 lg:pr-0 lg:w-5/6 md:mb-0">
          <div class="lg:pr-10">
            ...
            <div v-if="post.lastReplie" class="text-gray-800 text-xs">
              <Link class="text-blue-600 uppercase font-bold"    :href="route('profile', $page.props.auth.user.name)">{{ post.lastReplie.user.name }}</Link>
              replied
        ...
        <div class="relative hidden text-center md:ml-auto md:flex md:flex-row-reverse md:items-center"
          ...
          <div class="flex items-center justify-center ml-4">
            ...
            <span class="text-xs text-gray-800 font-semibold text-left leading-none relative">{{ post.count_reply
            }}</span>
          </div>
