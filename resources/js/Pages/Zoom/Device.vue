            ...
            <tbody>
                <tr v-for="(device, index) in devices.data" :key="index"
                    ...
                    <td class="px-6 py-4">
                        ...
                        <icon v-if="device.battery < 0.7 && device.battery > 0.2" name="battery-medium" class="w-8 h-8">
                        </icon>
...
export default {
    ...
    methods: {
        search() {
            this.$inertia.get(this.route('admin.device.index', { term: this.term }), {}, { preserveState: true })
        },
        listenForDevice() {
            var self = this;
            window.socketio.on("active-device:App\\Events\\ActiveDevice", function (e) {
                // console.log(e)
